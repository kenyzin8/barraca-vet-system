    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === 'Appointment cancelled successfully') {
                ...
                var tempAppointmentDate = new Date(response.appointment_date);
                var appointmentDateString = tempAppointmentDate.getFullYear() + '-' + String(tempAppointmentDate.getMonth() + 1).padStart(2, '0') + '-' + String(appointmentDate.getDate()).padStart(2, '0');
                _appointmentCounts[appointmentDateString] = _appointmentCounts[appointmentDateString] - 1;

                //console.log(_appointmentCounts[appointmentDateString]);
                refreshCalendar();
...
for (var i = 0; i < changeTimeButtons.length; i++) {
    changeTimeButtons[i].onclick = function() {
        ...
        var current_time_of_the_day = arg.event.extendedProps.timeOfTheDay_val;
        ...
        submitButton.onclick = function() {
            ...
            var client_id = arg.event.extendedProps.client_id;

            var baseUrl = "{% url 'set-time-of-the-day-client' 0 %}".slice(0,-1);

            var url = baseUrl + '{{ request.user.client.id }}' + '/';
